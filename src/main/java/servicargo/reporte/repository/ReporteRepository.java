package servicargo.reporte.repository;
...
public class ReporteRepository {
    ...
    public String reporteInventario() {
        String sql = "SELECT COUNT(*) AS items, COALESCE(SUM(cantidad),0) AS total_unidades FROM inventario";
        try (Connection conn = DbConnection.open();
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                return "Items en inventario: " + rs.getInt("items") + "\n" +
                    "Unidades totales: " + rs.getInt("total_unidades");
            }
